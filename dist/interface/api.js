...
import { CalculationUnavailableError } from "../calculate/service.js";
...
const error = (status, code, message) => response(status, {
    ok: false,
    error: { code, message },
});
...
const unix = (query, key, requiredValue) => {
    ...
    if (!Number.isSafeInteger(parsed) || parsed < 0)
        throw new Error(`${key} must be a non-negative Unix timestamp`);
    return parsed;
};
const inputFailure = (cause) => {
...
const calculation = async (request, runtime) => {
    let parsed;
    try {
...
const generation = async (request, runtime) => {
...
const trustedAuthority = (value, index) => {
    ...
    if (typeof issuer !== "string" || issuer.trim().length === 0)
        throw new Error(`trustedAuthorities[${index}].issuer is required`);
...
const validation = async (request) => {
    try {
        const raw = record(request.body, "request");
        if (!("file" in raw))
            throw new Error("request.file is required");
        const trustedRaw = raw["trustedAuthorities"];
        if (trustedRaw !== undefined && !Array.isArray(trustedRaw))
            throw new Error("request.trustedAuthorities must be an array");
...
const places = async (request, runtime) => {
    try {
        switch (request.path) {
            ...
            case "/v1/places/cities":
                return response(200, {
                    ok: true,
                    cities: await runtime.places.cities(required(request.query, "country"), request.query.get("region")?.trim() || null, request.query.get("q")?.trim() || ""),
                });
        ...
        if (/required/u.test(message))
            return error(400, "invalid_query", message);
        return error(404, "place_not_found", message);
    }
};
const billing = async (request, runtime) => {
    const bills = runtime.bills;
    if (bills === undefined)
        return error(503, "billing_not_configured", "Billing storage is not configured");
    try {
        switch (request.path) {
            case "/v1/billing":
            ...
            case "/v1/billing/provider-costs": {
                ...
                const start = unix(request.query, "start_time", true);
...
export const routeApi = async (request, runtime) => {
    ...
    if (request.path.startsWith("/v1/billing")) {
        if (method !== "GET")
            return error(405, "method_not_allowed", "Billing routes require GET");
        return billing(request, runtime);
    }
    if (request.path.startsWith("/v1/places/")) {
        if (method !== "GET")
            return error(405, "method_not_allowed", "Place routes require GET");
        return places(request, runtime);
    }
    if (request.path === "/v1/calculations") {
        if (method !== "POST")
            return error(405, "method_not_allowed", "Calculation route requires POST");
        return calculation(request, runtime);
    }
    if (request.path === "/v1/charts") {
        if (method !== "POST")
            return error(405, "method_not_allowed", "Chart generation requires POST");
        return generation(request, runtime);
    }
    if (request.path === "/v1/files/validate") {
        if (method !== "POST")
            return error(405, "method_not_allowed", "File validation requires POST");
