...
const compatibilityShape = (file) => {
    const calculation = file["astral-calculation"];
    const matrix = calculation.compatibility;
    if (matrix.zodiac !== calculation.system.zodiac)
        return false;
    for (const domain of compatibilityDomains) {
        const scores = matrix.domains[domain];
        if (scores.domain !== domain || !compatibilityValid(scores))
            return false;
    }
    return true;
};
const interpretationPlanShape = (file) => {
    const calculation = file["astral-calculation"];
    const plan = calculation.interpretationPlan;
    if (plan.schema !== "astral-interpretation-plan/1.1.0" || !Array.isArray(plan.units))
        return false;
    if (plan.zodiac !== calculation.system.zodiac)
        return false;
...
const chartShape = (file) => {
    const calculation = file["astral-calculation"];
    const chart = file["astral-chart"];
    if (chart.schema !== "astral-chart/1.1.0")
        return false;
    if (chart.zodiac !== calculation.system.zodiac)
        return false;
    if (chart.system.zodiac !== chart.zodiac || chart.compatibility.zodiac !== chart.zodiac)
        return false;
    const name = chart.subject.name;
    if (name.source === "generated" && !generatedNamePattern.test(name.value))
        return false;
    return name.value.length > 0 && Array.isArray(name.sourceRefs);
};
export const isAstralFile = (value) => {
    if (!record(value))
        return false;
    const file = value;
    try {
...
export const isLegacyAstralFile = (value) => {
...
export const parseAstralFile = (value) => {
    if (!isAstralFile(value))
        throw new TypeError("Value is not a structurally valid astral/1.1.0 file");
    return value;
};
export const parseReadableAstralFile = (value) => {
    if (isAstralFile(value) || isLegacyAstralFile(value))
        return value;
...
export const decodeAstralFile = (text) => parseAstralFile(decode(text));
export const decodeReadableAstralFile = (text) => parseReadableAstralFile(decode(text));
export const encodeAstralFile = (file, pretty = false) => {
    parseAstralFile(file);
    return pretty ? `${JSON.stringify(file, null, 2)}\n` : canonicalise(file);
};
const authorityStatus = async (file, trusted) => {
    ...
    if (known.status === "revoked")
        return "revoked";
    if (known.issuer !== authority.issuer || known.publicKey !== authority.publicKey)
        return "unknown_key";
    return known.status === "active" ? "trusted" : "valid_untrusted";
};
export const validateAstralFile = async (value, trusted = []) => {
