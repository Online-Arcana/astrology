...
const angles = coreAngles(geometry, 0, 0);

await test("core angles match the synthetic equatorial fixture", () => {
  close(angles.localSiderealDegrees, 75.75311289815316, 1e-9, "local sidereal time");
  close(angles.ascendant, 164.53056064550913, 1e-7, "Ascendant");
  close(angles.midheaven, 76.88656755496957, 1e-7, "Midheaven");
  close(angles.descendant, 344.5305606455091, 1e-7, "Descendant");
  close(angles.imumCoeli, 256.8865675549696, 1e-7, "IC");
});

await test("Placidus semi-arc roots match the independent reference", () => {
  const cusps = placidusCusps(angles, 0, geometry.trueObliquityRadians);
...
await test("house placement reports cusp distance and intercepted status", () => {
  ...
  close(placement.value?.distanceFromCuspDegrees ?? -1, 2.90927497766344, 1e-6, "cusp distance");
});
