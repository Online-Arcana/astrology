...
await test("JSON router exposes health and strict method handling", async () => {
  ...
  equal((health.body as { version: string }).version, "0.11.0", "health version");
...
await test("CLI parser keeps stdin stdout defaults and validates choices", () => {
...
await test("HTTP adapter serves JSON on an ephemeral localhost port", async () => {
  const { server, address } = await listenAstralServer(runtime, { port: 0 });
  try {
    const health = await fetch(`http://127.0.0.1:${address.port}/health`);
    equal(health.status, 200, "HTTP health status");
    equal((await health.json() as { version: string }).version, "0.11.0", "HTTP health version");
