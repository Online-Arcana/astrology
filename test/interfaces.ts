...
await test("request parser applies defaults and validates unknown time", () => {
  const exact = parseCalculationRequest({
    birth: { date: "2000-06-15", time: "12:30:00", timeAccuracy: "exact", placeId: place.id },
  }, options);
  equal(exact.options.ayanamsha, "lahiri", "default ayanamsha");
  const unknown = parseCalculationRequest({
    birth: { date: "2000-06-15", time: null, timeAccuracy: "unknown", placeId: place.id },
  ...
  try {
    parseCalculationRequest({
      birth: { date: "2000-06-15", time: "12:00:00", timeAccuracy: "unknown", placeId: place.id },
    }, options);
  } catch {
    failed = true;
  }
