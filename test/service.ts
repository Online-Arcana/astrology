...
const timeResolver: TimeResolver = {
  ...
  resolve: ({ date, time }) => {
    ...
    if (date === "2000-06-15" && time === "12:30:00") {
      return {
        kind: "exact",
        localIso,
        candidate: {
    ...
    if (date === "2000-06-15" && time === "00:00:00") {
      return {
        kind: "exact",
        localIso,
        candidate: {
          fold: null,
          utcIso: "2000-06-14T23:00:00Z",
          offsetSeconds: 3600,
          daylightSaving: true,
        },
      };
    }
    if (date === "2000-06-16" && time === "00:00:00") {
      return {
        kind: "exact",
        localIso,
        candidate: {
          fold: null,
          utcIso: "2000-06-15T23:00:00Z",
          offsetSeconds: 3600,
          daylightSaving: true,
        },
      };
...
const isoJde: Readonly<Record<string, number>> = {
  "2000-06-14T23:00:00Z": referenceJde - 0.5208333333,
  "2000-06-15T11:30:00Z": referenceJde,
  "2000-06-15T23:00:00Z": referenceJde + 0.4791666667,
};
...
const eclipses: EclipsePort = {
  ...
  decimalYear: () => 2000.45,
  utcIso: () => "2000-01-01T00:00:00.000Z",
};
