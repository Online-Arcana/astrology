...
`vendor/time` is pinned to `kitty-crow/ts-joda`, the maintained drop-in fork of js-joda. The project uses `@js-joda/core` 6.1.0 and `@js-joda/timezone` 2.25.2 behind the strict `TimeResolver` boundary in `src/time`.
...
The pinned timezone package contains IANA tzdb 2026a rules rather than delegating historical results to the host runtime. The fork repository, submodule revision, package versions and tzdb version are recorded in calculation provenance.
...
1. Update the `kitty-crow/ts-joda` submodule deliberately.
2. Record the new core, timezone and timezone-data versions.
3. Run the complete vendor build and integration suite before accepting a new revision.
4. Run transition fixtures, historical-offset fixtures and pre-standard-time fixtures.
5. Change the supported range only when the evidence changes.
